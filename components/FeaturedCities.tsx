    ...
    <section id="ciudades" className="py-24 bg-muted-bg">
      <div className="max-w-6xl mx-auto px-5">
        <div className="text-center mb-12">
          ...
          <TextReveal
            text="Ciudades donde Nidoo ya conecta roommates"
            as="h2"
        ...
        {/* City grid */}
        <Stagger className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-12" stagger={0.1}>
          {cities.map((city) => (
            <motion.div key={city.name} variants={staggerItemScale}>
              <Link
                href={`/explorar?ciudad=${city.name.toLowerCase()}`}
                className="group relative aspect-[3/4] rounded-2xl overflow-hidden shadow-sm hover:shadow-primary-lg transition-all duration-300 block"
              >
                {/* Background image */}
                <Image
                  src={city.image}
                  ...
                  <div className="glass rounded-lg px-3 py-2 mt-2">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-foreground font-semibold text-sm">{city.listings}</p>
                      </div>
                    </div>
                  </div>
                </div>
